		...
		<scroll-view scroll-x="true" class="navigation">
			<view class="bg">
				<view v-for="itme,index in tita" :key="index" @click="dianji_qiehuan(index)" style="margin: auto;">
					<view class="box1" :style="index==daohangValue?'color:#3999FE;':''">{{itme}}</view>
				</view>
				<view class="xiabiao"
					:style="daohangValue==0 ? 'left:80rpx;' : daohangValue==1 ?'left:330rpx;':'left:580rpx;'">
				</view>
			</view>
		</scroll-view>
	...
	export default {
		...
		methods: {
			...
			dianji_qiehuan(index) {
				this.daohangValue = index
				if (index == 0) {
					this.queryParam.classify = "1"
				} else if (index == 1) {
					this.queryParam.classify = "2"
				} else if (index == 2) {
					this.queryParam.classify = "3"
				}
				this.init()
			},
			siftingBtn(item) {
